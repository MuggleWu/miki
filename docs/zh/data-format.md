...
- 压实时 delta **只留最后一行当锚点**（既不是删除，也不是清空）。多端同步下，「这一端删掉 delta
  文件」撞上「另一端往同一 delta 追加」在 git 里是 modify/delete **硬冲突**——git 不会自动合，每次
  pull 都得人工判一次语义；清空（0 行）同样不行，实测 git 会把「删光 1..N 行」与「在第 N 行后追加
  M 行」当成同一处重叠改动，照样 CONFLICT。只留最后一行后，两处改动落在不相邻的 hunk（本地只删
  1..N-1，远端在 N 之后插入），git 自动合并成「锚点 + 远端那 M 行」。锚点行的内容早已折进基文件，
  被重放覆盖一次是幂等的，且顺序上锚点在前、远端新行在后，后写覆盖仍取远端的新内容。
  读取端按**行数**判断待压实内容，所以锚点行不会变成每次启动都重写基文件。
- 启动时还会**结算历史遗留的积压**：加载时在 delta 合并前快照基文件的删除标记，重放后与
  内存态比对，某牌组「已删但基文件仍显示存活」达到 20 张就自动压实一次（幂等，无积压不写盘）。
  这条是为修复之前累积的删除准备的——那时压实路径走不到，删除可以永远不落盘。
...
- 内容覆盖行**比基行旧**（`updatedAt` 更小）时整行内容都不认：基行上那份才是更新的版本，旧行只是
  「运行实例把内存里的旧卡面写回」留在 delta 里的残留（2026-09-20 定案——这类行曾把 37 张卡的
  KaTeX 背面盖回旧纯文本）。调度快照、暂停态与水位仍照旧继承基行；带水位的移入行不受这条判据影响。
