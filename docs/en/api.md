...
- Address: `http://127.0.0.1:<port>/api`; the port defaults to `8727` (incrementing if taken; the actual port appears in the startup log and is written to the `userData/miki-api.json` runtime file with `port` and `pid`)
...
| `dueAfter` / `dueBefore` | Due-time window: ms epoch or a date string (e.g. `2026-09-30`); cards without scheduling progress are never in the window |
...
- MCP integration: `node scripts/mcp-server.mjs`. **Auto-discovery is recommended**: with no `MIKI_TOKEN`, the wrapper locates the current workspace (`MIKI_WORKSPACE` env → `current` in `userData/workspace.json`) and reads its `config.json` for the token; the port prefers the `userData/miki-api.json` runtime file — after switching workspaces, just restart the MCP to follow along. Explicit `MIKI_TOKEN` (+ optional `MIKI_PORT`) keeps the legacy behavior. Tools include `list_decks` / `add_cards` / `search_cards` (default page size 50; date params accept `YYYY-MM-DD`) / `update_cards` (partial update) / `rename_deck` / `set_suspended` (batch) / `api_schema`, plus read-only resources `miki://decks`, `miki://stats` and the `make-cards` prompt template (minimum-information principle).
