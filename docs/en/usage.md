...
- Queue order: learning queue (minute-level steps, slots back in when due) → review cards due right now → new cards. Only what is due *right now* is served: a review card due later today neither jumps the queue nor gets pulled forward as a fallback, so once the due cards are cleared the new cards start. Learning comebacks slot back in when due without blocking new cards. Daily reviews are unlimited.
