...
- Compaction **keeps only the last delta line as an anchor** (neither deleting nor truncating). With multi-device sync, "this side deletes the delta file" colliding with "the other side appends to the same delta" is a git modify/delete **hard conflict** — git will not auto-merge it, so every pull needs a manual semantic call. Truncating (0 lines) does not help either: git treats "delete lines 1..N" and "append M lines after line N" as one overlapping change and still reports CONFLICT. Keeping only the last line puts the two edits in non-adjacent hunks (local deletes 1..N-1, remote inserts after N), which git auto-merges into "anchor + the remote's M lines". The anchor's content already reached the base file, so replaying it once is idempotent; and since the anchor comes first and the remote's new lines follow, last-write-wins still yields the remote's content. The reader decides what is pending by **row count**, so the anchor never turns into a base-file rewrite on every startup.
